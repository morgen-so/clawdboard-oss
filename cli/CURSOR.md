...
Extraction is implemented in [`src/cursor.ts`](src/cursor.ts), which reads
Cursor's local SQLite database (`state.vscdb`) on disk. It emits
`source: "cursor"` and runs from [`extract.ts`](src/extract.ts) alongside the
other per-source extractors.

This covers Cursor's **legacy local format** (roughly April 2025 through
mid-September 2025), where token counts and per-composer cost were persisted
locally. Around `_v: 14`, Cursor moved conversation content (and per-bubble
token counts) server-side; recovering that period requires Cursor's account
API and is a **planned follow-up** — see [Future work](#future-work) below.
...
The local SQLite extractor only sees Cursor data from before Cursor's
~Sep 2025 server-side migration. Post-cliff usage lives behind Cursor's
own services and is out of scope for clawdboard — there is no
personal-tier usage API available today, and clawdboard will not ship
tooling that reads private endpoints or scrapes auth tokens from other
applications' storage. If Cursor publishes a supported personal-tier
usage API in the future, that path can be added as a follow-up.
