...
Track and compare your AI coding agent usage across developers. Supports Claude Code, the Claude desktop app (including its Cowork and Dispatch agent sessions), OpenCode (incl. Go and Zen tiers), Codex CLI, Cursor, Gemini CLI, GitHub Copilot CLI, Antigravity (opt-in), Pi, Hermes Agent, and DeepSeek Harness. See who's spending the most, longest streaks, model breakdowns, and more.
...
2. **Extract** — Reads your local usage logs from each supported tool (`~/.claude/`, `~/.local/share/opencode/`, `~/.codex/`, Cursor's `state.vscdb`, `~/.gemini/`, `~/.copilot/`, `~/.pi/agent/sessions/`, `~/.hermes/state.db`, `~/.dsh/sessions/`, and the Claude desktop app's session directory on macOS)
